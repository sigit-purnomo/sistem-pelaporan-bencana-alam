...
class C_petugas extends CI_Controller {
    ...
    function lists() {
        if($this->session->userdata('logged_in'))
        {
            ...
            $buttons[] = array('separator');
            $buttons[] = array('Tambah Petugas','add','btn');
            $buttons[] = array('separator');
            $buttons[] = array('Hapus Petugas','delete','btn');
            $buttons[] = array('separator');
            ...
            $grid_js = build_grid_js('flex1',site_url('admin/c_petugas/load_data'),$colModel,'id_user','asc',$gridParams,$buttons);

            $data['js_grid'] = $grid_js;

            $data['page_title'] = 'Pengelolaan Data Petugas';
            $data['content'] = $this->load->view('petugas/v_list', $data, TRUE);
    ...
    function load_data() {	
        if($this->session->userdata('logged_in'))
        {
            ...
            $records = $this->m_petugas->get_petugas_flexigrid();
            ...
            foreach ($records['records']->result() as $row)
            {
                $record_items[] = array(
                    ...
                    $row->username,
                    $row->no_hp,
                    '<button type="submit" class="btn btn-default btn-xs" title="Ubah petugas" onclick="edit_petugas(\''.$row->id_user.'\')"/><i class="fa fa-pencil"></i></button>
                    <button type="submit" class="btn btn-default btn-xs" title="Reset Password Petugas" onclick="reset_petugas(\''.$row->id_user.'\')"/>Reset Password</i></button>'
                );  
            }
            //Print please
    ...
    function add(){
        if($this->session->userdata('logged_in'))
        {           
            $data['page_title'] = 'Tambah Petugas';
            $data['content'] = $this->load->view('petugas/v_tambah', $data, TRUE);
    ...
    function simpan_petugas() {
        if($this->session->userdata('logged_in'))
        {
            ...
            {      
                ...
                $this->m_petugas->insertPetugas($data);
                redirect('admin/c_petugas','refresh');
            }
            else $this->add();
        }else{
            redirect('c_login','refresh');
    ...
    function delete()    {
        if($this->session->userdata('logged_in'))
        {
            ...
            $data = array(
                ...
                $this->m_petugas->deletepetugas($post,$data);          
                redirect('admin/c_petugas', 'refresh');
        }else{
            redirect('c_login','refresh');
        }
    ...
    function edit_petugas($id_user){
        if($this->session->userdata('logged_in'))
        {
            $data['data_petugas'] = $this->m_petugas->getPet($id_user);
            $data['id_user'] = $id_user;

            $data['page_title'] = 'Ubah Data Petugas';
            $data['content'] = $this->load->view('petugas/v_ubah', $data, TRUE);
    ...
    function reset_petugas($id_user){
        if($this->session->userdata('logged_in'))
        {
    ...
    function update_petugas(){
        if($this->session->userdata('logged_in'))
        {
            ...
            $this->form_validation->set_rules('no_hp', 'Nomor Handphone', 'callback_isduplicatehpubah|required');
            ...
            if ($this->form_validation->run() == TRUE)
            {
                ...
                $result = $this->m_petugas->updatepetugas($id_user, $data);
                if($result>0){
                    redirect('admin/c_petugas','refresh');
                }else{
                    redirect('admin/c_petugas','refresh');
            ...
            else $this->edit_petugas($id_user);
        }else{
            redirect('c_login','refresh');
        }
